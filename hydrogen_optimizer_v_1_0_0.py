# e-Hydrogen Cost Optimizer by KAUST (v1.0.0)
#------------------------------------------------------------------------------------------------------
# Apache-2.0 License
#------------------------------------------------------------------------------------------------------
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.title("e-Hydrogen Cost Optimizer by KAUST (v.1.0.0)")
        ...
        self.opt_seg_button_1.configure(values=["Guarantee an Hourly Demand","Guarantee a Daily Demand", "Guarantee a Yearly Demand"])
        ...
        self.textbox.insert("0.0", "Welcome to the e-Hydrogen Cost Optimizer developed by KAUST! \n\n"
                             ...
                             +" 6. Go to the LCOE Analysis tab to calculate the Levelized Cost of Electricity (LCOE) of the system.\n"
                             +" 7. Go to the LCOH Analysis tab to view in detail how the LCOH is distributed among the components used in the outcome model.\n"
                             +" 8. Go to the Renewables tab to analyse the distribution of both Solar PV and Wind power of the location selected.\n"
                             +" 9. In order to explore further the outcome model, please go to the root folder of the app /_internal/modelOutputs/results.xlsx\n"
                             +" 10. Go to the Life Cycle Assessment tab to perform an LCA and view the contribution analysis of the components used in the outcome model.\n\n"
                             +" Notes:\n"
    ...
    def pyomo_opt(self):
        ...
        if self.daily_constraint == 1:
            # Meet the hydrogen demand on an hourly basis (Daily H2 demand / 24)
            def meetHydrogenDemand(model, t):
                    return (
                        sum(model.eyGen[i, t] for i in model.eyPlants)
                        - model.hsStore[t] 
                        + model.hsDeploy[t]
                        == model.Hydrogen_Demand/24
                    )
            model.meetHydrogenDemandConstraint = Constraint(model.horizon, rule=meetHydrogenDemand)
        elif self.daily_constraint==2:
            #meet the hydrogen production targets in a daily basis (Daily H2 demand)
        ...
        elif self.daily_constraint==3:
            #meet the hydrogen production targets in a yearly basis (Daily H2 demand * 365)
...
###
        ...
        # Use constrained_layout=False here to avoid "axes sizes collapsed to zero" warnings
        # from matplotlib when the canvas size is not yet available in a Tkinter embed.
        #fig6 = plt.figure(figsize=(10,5), dpi=72, constrained_layout=False)
        fig6 = plt.figure(figsize=(9,6),dpi=72)
        ax6 = fig6.add_subplot(1,1,1, xticks=[], yticks=[])
        ax6.set_title("LCA Contribution Analysis", fontsize=10)
    ...
    def optimization(self):
        self.update_label_status("Optimization in progress...")
        if self.opt_seg_button_1.get()=='Guarantee an Hourly Demand':
            self.daily_constraint = 1
        elif self.opt_seg_button_1.get()=='Guarantee a Daily Demand':
            self.daily_constraint = 2
        elif self.opt_seg_button_1.get()=='Guarantee a Yearly Demand':
            self.daily_constraint = 3
        #start the thread for the optimization tasks
        self.optimization_thread = threading.Thread(target=self.run_optimization)
        self.optimization_thread.start()
        # Start periodic checking
    ...
    def on_close(self):
        # Safely close Matplotlib figures
        plt.close('all')

        # Destroy Tkinter widgets
        self.destroy()

        # Exit the process explicitly if needed
        import sys
        sys.exit(0)
